...
function initTables() {
  ...
  ensureColumn('admins', 'totp_secret', 'TEXT');
  ensureColumn('admins', 'totp_enabled', 'INTEGER NOT NULL DEFAULT 0');
}
...
const adminOps = {
  ...
  getSecurity(id) {
    return getDb().prepare('SELECT id, username, totp_enabled, totp_secret FROM admins WHERE id = ?').get(id);
  },
  count() {
  ...
  enableTotp(id, secret) {
    return getDb().prepare('UPDATE admins SET totp_secret = ?, totp_enabled = 1 WHERE id = ?').run(secret, id);
  },
  disableTotp(id) {
    return getDb().prepare('UPDATE admins SET totp_secret = NULL, totp_enabled = 0 WHERE id = ?').run(id);
  }
};
